let square = {
  ...
  speedX: 3,
  speedY: 3,
  minSpeed: 1,    // Add minimum speed limit
  maxSpeed: 20     // Add maximum speed limit
};
...
function draw() {
  ...
  // Update timer and set new target sizes and speeds occasionally
  changeTimer++;
  if (changeTimer > 60) { // Change every 60 frames (about 1 second)
    ...
    // Add random speed changes
    square.speedX = random(square.minSpeed, square.maxSpeed) * (square.speedX > 0 ? 1 : -1);
    square.speedY = random(square.minSpeed, square.maxSpeed) * (square.speedY > 0 ? 1 : -1);
  }
