...
let targetWidths = [60, 60, 60];  // One for each square
let targetHeights = [60, 60, 60]; // One for each square
let changeTimer = 0;
...
function draw() {
  ...
  if (changeTimer > 60) {
    changeTimer = 0;
    for (let i = 0; i < 3; i++) {
      targetWidths[i] = random(30, 200);   // Smaller max size for each square
